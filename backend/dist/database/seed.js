...
async function main() {
    ...
    await db_1.prisma.redemption.deleteMany();
    await db_1.prisma.reward.deleteMany();
    await db_1.prisma.trip.deleteMany();
    await db_1.prisma.report.deleteMany();
    await db_1.prisma.rewardItem.deleteMany();
    await db_1.prisma.user.deleteMany();
    const adminPassword = await bcryptjs_1.default.hash('Admin@12345', 10);
    const userPassword = await bcryptjs_1.default.hash('Green@12345', 10);
    // 1. Create Admin
    ...
    // 2. Create Demo Users
    const alex = await db_1.prisma.user.create({
        data: {
            name: 'Alex Rivera',
    ...
    const priya = await db_1.prisma.user.create({
        data: {
            name: 'Priya Sharma',
    ...
    const marcus = await db_1.prisma.user.create({
        data: {
            name: 'Marcus Lind',
    ...
    for (const t of sampleTrips) {
        ...
        await db_1.prisma.reward.create({
            data: {
                userId: alex.id,
    ...
    const aiInsights = (0, ibmGraniteService_1.generateMonthlyAIReportInsights)(alex.name, sampleTrips.length, totalSaved, 'Bicycle');
    await db_1.prisma.report.create({
        data: {
            userId: alex.id,
            month: 'August 2026',
