...
exports.manageRewards = manageRewards;
exports.createRewardItem = createRewardItem;
...
async function getAdminMetrics(req, res) {
    try {
        ...
        const totalTrips = await db_1.prisma.trip.count();
        ...
        const activeUsersCount = await db_1.prisma.user.count({
            where: {
                trips: { some: {} },
            },
        });
...
async function manageRewards(req, res) {
    try {
        ...
        const recentRedemptions = await db_1.prisma.redemption.findMany({
            include: {
...
async function createRewardItem(req, res) {
    try {
        ...
        const item = await db_1.prisma.rewardItem.create({
            data: {
                title,
...
async function toggleRewardAvailability(req, res) {
    try {
        const { itemId } = req.params;
        const { available } = req.body;
        const updated = await db_1.prisma.rewardItem.update({
            where: { id: itemId },
