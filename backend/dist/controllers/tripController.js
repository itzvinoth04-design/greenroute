...
exports.recordTrip = recordTrip;
exports.getUserTrips = getUserTrips;
exports.getEcoInsights = getEcoInsights;
const db_1 = require("../database/db");
const constants_1 = require("../config/constants");
async function recordTrip(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: 'Unauthorized' });
            return;
        }
        const { source, destination, distance, duration, cost, transportType, carbonEmission, carbonSaved, sustainabilityScore, trafficDensity, } = req.body;
        ...
        const pointsAwarded = constants_1.TRANSPORT_MODES[mode]?.pointsPerTrip ?? 0;
        // Use transaction to create trip, award points, and add reward event
        const [trip, user] = await db_1.prisma.$transaction([
            db_1.prisma.trip.create({
                data: {
                    userId: req.user.id,
                    source,
                    destination,
...
async function getEcoInsights(req, res) {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: 'Unauthorized' });
            return;
        ...
        const modeDistribution = Object.entries(modeCounts).map(([mode, data]) => ({
            ...
            color: constants_1.TRANSPORT_MODES[mode]?.color || '#10b981',
        }));
