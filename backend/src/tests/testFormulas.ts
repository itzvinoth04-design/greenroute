import {
  ...
  computeRouteMetrics,
} from '../services/emissionCalculator';
import { rankAndScoreRoutes } from '../services/scoringEngine';
...
function runTests() {
  ...
  const expectedFactors: Record<TransportMode, number> = {
    Walking: 0.0,
    Bicycle: 0.0,
    Metro: 0.4,       // 10 * 0.04
    Bus: 0.8,         // 10 * 0.08
  ...
  for (const [mode, expectedEmission] of Object.entries(expectedFactors) as [TransportMode, number][]) {
    const calc = calculateCarbonEmission(distance, mode);
    console.assert(
      Math.abs(calc - expectedEmission) < 0.001,
      `Emission mismatch for ${mode}: expected ${expectedEmission}, got ${calc}`
    );
  ...
  const metroSaved = calculateCarbonSaved(10, 'Metro');
  ...
  const bikeSaved = calculateCarbonSaved(10, 'Bicycle');
  ...
  const metrics = modes.map((m) => computeRouteMetrics({ distanceKm: 10, mode: m, trafficDensity: 'Moderate' }));
  const ranked = rankAndScoreRoutes(metrics);

  console.log('Ranked Recommendations Output:');
  ranked.forEach((r) => {
    console.log(
      ` Rank #${r.rank} [Score: ${r.sustainabilityScore}/100] ${r.mode.padEnd(12)} - CO2: ${r.carbonEmissionKg}kg, Cost: $${r.costEstimate}, Time: ${r.travelTimeMinutes}m (${r.badge || 'Alternative'})`
    );
  });
